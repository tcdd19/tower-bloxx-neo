...
let text = fs.readFileSync('game.js', 'utf8');
...
`;
text = text.replace(/(this\.ctx = this\.canvas\.getContext\('2d'\);)/, `$1` + init_add);

// 3. Modify startGame to clear sprite effects
text = text.replace(/(this\.floatingTexts = \[\];)/, `$1\n    this.spriteEffects = [];`);
...
`;
text = text.replace(/(\/\/ 1\. 粒子物理更新)/, update_code + '\n    $1');
...
`;
text = text.replace(/(\/\/ E4: 居民增加)/, landing_code + '\n    $1');
...
text = text.replace(/  drawSkyscraperSilhouettes\(\) \{[\s\S]*?(?=  \/\/ A5\+D1)/, draw_bg_code + '\n');
text = text.replace(/    \/\/ 1\.5 诺基亚原版高楼背景剪影[^\n]*\n\s*this\.drawSkyscraperSilhouettes\(\);\n/, '');

// 7. Modify drawParkSilhouettes
const draw_park_code = `
  drawParkSilhouettes() {
...
`;
text = text.replace(/  drawParkSilhouettes\(\) \{[\s\S]*?(?=  drawParallaxStarsAndClouds\(\) \{)/, draw_park_code + '\n');
...
`;
text = text.replace(/      this\.ctx\.save\(\);\n      \/\/ 尖顶红三角[\s\S]*?this\.ctx\.restore\(\);\n/, roof_code);
...
`;
text = text.replace(/  drawCrane\(\) \{[\s\S]*?(?=  \/\/ 圆角矩形辅助算法)/, crane_code + '\n');
...
`;
text = text.replace(/(    this\.floatingTexts\.forEach\(txt => txt\.draw\(this\.ctx, 'Share Tech Mono', 'Outfit'\)\);\n)/, `$1\n${badge_code}`);

// 11. Add loadAll trigger to initGameEngine
const init_game_code = `
...
`;
text = text.replace(/function initGameEngine\(\) \{[\s\S]*?(?=if \(document\.readyState)/, init_game_code + '\n');
...
`;
text = text.replace(/  loop\(timestamp\) \{\n    \/\/ D2: 页面隐藏时暂停逻辑\n    if \(this\._pageHidden\) \{\n      requestAnimationFrame\(this\.loop\);\n      return;\n    \}/, loop_code);

fs.writeFileSync('game.js', text);
